...
class account_invoice(osv.osv):
    ...
    _columns = {
        ...
        'intrastat_transaction_id': fields.many2one(
            'l10n_hr_intrastat.transaction', 'Intrastat Transaction Type',
            help="Intrastat nature of transaction"),
        'transport_mode_id': fields.many2one(
            'l10n_hr_intrastat.transport_mode', 'Intrastat Transport Mode'),
...
class intrastat_region(osv.osv):
    _name = 'l10n_hr_intrastat.region'
    _columns = {
    ...
    _sql_constraints = [
        ('l10n_hr_intrastat_regioncodeunique', 'UNIQUE (code)', 'Code must be unique.'),
    ]


class intrastat_transaction(osv.osv):
    _name = 'l10n_hr_intrastat.transaction'
    _rec_name = 'code'
    _columns = {
    ...
    _sql_constraints = [
        ('l10n_hr_intrastat_trcodeunique', 'UNIQUE (code)', 'Code must be unique.'),
    ]


class intrastat_transport_mode(osv.osv):
    _name = 'l10n_hr_intrastat.transport_mode'
    _columns = {
        'code': fields.char('Code', required=True, readonly=True),
    ...
    _sql_constraints = [
        ('l10n_hr_intrastat_trmodecodeunique', 'UNIQUE (code)', 'Code must be unique.'),
    ]
...
class res_company(osv.osv):
    ...
    _columns = {
        'region_id': fields.many2one('l10n_hr_intrastat.region', 'Intrastat region'),
        'transport_mode_id': fields.many2one('l10n_hr_intrastat.transport_mode',
