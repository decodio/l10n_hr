...
class account_invoice(osv.Model):
    ...
    _columns = {
                ...
                'uredjaj_id':fields.many2one('fiskal.uredjaj', 'Naplatni uredjaj', help ="Naplatni uređaj na kojem se izdaje racun", readonly=True, states={'draft':[('readonly',False)]}),
    ...
    def fiskaliziraj(self, cr, uid, id, context=None):
        ...
        if not dat_vrijeme:
            ...
            #update account_invoice set vrijeme_izdavanja = '2013-07-17 12:10:10' where id=112
        else:
            tstamp = datetime.strptime(dat_vrijeme,'%Y-%m-%d %H:%M:%S')
            tstamp = timezone('UTC').localize(tstamp).astimezone(timezone('Europe/Zagreb'))
            dat_vrijeme = '%02d.%02d.%02dT%02d:%02d:%02d' % (tstamp.day, tstamp.month, tstamp.year, tstamp.hour, tstamp.minute, tstamp.second)
