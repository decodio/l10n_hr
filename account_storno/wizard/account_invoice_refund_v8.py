...
class AccountInvoice(models.Model):
    ...
    def _prepare_refund(self, invoice, date=None, period_id=None, description=None, journal_id=None):
        values = super(AccountInvoice, self)._prepare_refund(invoice, date=date,
                                              period_id=period_id, description=description,
                                               journal_id=journal_id)
        values['entry_type'] = invoice.entry_type
        if invoice.__contains__('advance_payment') and invoice.advance_payment:
            values['advance_payment'] = invoice.advance_payment
        return values
...
class account_invoice_refund(orm.TransientModel):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        ...
        res = super(account_invoice_refund, self).fields_view_get(cr, uid, view_id=view_id,
        ...
        journal_select = journal_obj._name_search(cr, uid, '', [('type', 'in', journal_types),
                                                                ('company_id', 'child_of', [company_id])],
                                                  context=context)
        # original for loop needed???
        res['fields'].update({'journal_id' : {'selection' : journal_select}})
        return res
