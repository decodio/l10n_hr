...
class cancel_vat_settlement(report_sxw.rml_parse):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        self.journal_id = data['form']['property_cancel_vat_close_journal']
        self.closing_date = data['form']['closing_date']
        self.vat_account_payable = data['form']['property_cancel_vat_account_payable']
        self.vat_account_receivable = data['form']['property_cancel_vat_account_receivable']
        res['periods'] = ''
        ...
        if data['form'].get('period_from', False):
            ...
            for period in periods_l:
                if res['periods'] == '':
                    ...
                    res['periods'] += ", " + period['name']
        return super(cancel_vat_settlement, self).set_context(objects, data, new_ids, report_type=report_type)

    def __init__(self, cr, uid, name, context=None):
        super(cancel_vat_settlement, self).__init__(cr, uid, name, context=context)
    ...
    def _get_lines(self, based_on, company_id=False, parent=False, level=0, context=None):
        ...
        res2 = []
        while i < len(res):
            if res[i][1].id in tax_codes_list:
        ...
        self._cancel_vat_settlement(self.cr, self.uid, res2, top_result, period_list, company_id, context=context)
        return top_result
    ...
    def _get_general(self, tax_code_id, period_list, company_id, based_on, context=None):
        ...
        self.cr.execute('SELECT SUM(line.tax_amount) AS tax_amount, \
                ...
                WHERE line.state <> %s \
                    ...
                    AND account.active \
                    AND line.closed_vat IS %s \
                GROUP BY account.id,account.name,account.code, line.tax_code_id  \
    ...
    def _get_codes(self, based_on, company_id, parent=False, level=0, period_list=[], context=None):
        ...
        ids = obj_tc.search(self.cr, self.uid, [('parent_id', '=', parent),
                                                ('company_id', '=', company_id),
                                                ('payment_tax_code_id', '=', False)],
    ...
    def prepare_vat_lines(self, cr, uid, move_id, journal_id, account_id, 
    ...
    def prepare_vat_counterpart_lines(self, cr, uid, move_id, journal_id, counter_account,
                          debit_amount, credit_amount, closing_date, period, context=None):
    ...
    def _cancel_vat_settlement(self, cr, uid, tax_code_id, data, period_list, company_id, context=None):
        if context is None:
            context = {}
        ...
        tax_receivable = tax_payable = 0.0
        new_lines = []
        counter_account = False
        for tax_acc in tax_grouped.values():
            if tax_acc['debit'] != 0:
                tax_payable += tax_acc['debit']
            elif tax_acc['credit'] != 0:
                tax_receivable += tax_acc['credit']

            new_line = self.prepare_vat_lines(cr, uid, new_move_id, self.journal_id, tax_acc['account'].id, tax_acc['debit'], tax_acc['credit'], self.closing_date, period, context=None)            
        ...
        if tax_payable != 0.0:
            ...
            new_counter_lines = self.prepare_vat_counterpart_lines(cr, uid, new_move_id, self.journal_id, counter_account, tax_payable, 0.0, self.closing_date, period, context=None)            
            new_lines.append(new_counter_lines)
        if tax_receivable != 0.0:
            counter_account = self.vat_account_receivable
            new_counter_lines = self.prepare_vat_counterpart_lines(cr, uid, new_move_id, self.journal_id, counter_account, 0.0, tax_receivable, self.closing_date, period, context=None)         
            new_lines.append(new_counter_lines)
        if new_lines:
            for line in new_lines:
                move_line_obj.create(cr, uid, line, context)            
        self.cr.execute('UPDATE account_move_line AS line SET closed_vat = False \
                        WHERE line.state <> %s \
...
report_sxw.report_sxw('report.cancel.vat.settlement', 'account.tax.code',
    'addons/l10n_hr_vat/report/account_vat_settlement_report.rml', parser=cancel_vat_settlement, header="internal")
